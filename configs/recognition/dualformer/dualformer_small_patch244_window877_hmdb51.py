...
model=dict(backbone=dict(drop_path_rate=0.3, video_size=(32, 224, 224), num_classes=51), 
...
test_pipeline = [
    ...
    dict(type='ThreeCrop', crop_size=224), # you can change to centercrop to achieve 4 x 1 views, 3x lower FLOPs
    dict(type='Flip', flip_ratio=0),
    dict(type='Normalize', **img_norm_cfg),
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
...
work_dir = './work_dirs/hmdb51_split{}/dualformer_small_patch244_window877'.format(split)
find_unused_parameters = False
# resume_from = work_dir + '/latest.pth'
load_from = 'checkpoints/k400/dualformer_small_patch244_window877.pth'

...
model = dict(type='Recognizer3D_TL', # using token labelling on the fly, mix-token is disabled
             backbone=dict(drop_path_rate=0.1),
             cls_head=dict(type='I3DHead_TL',
                           in_channels=512,
                           loss_cls=dict(type='SoftCE_TL'),
                           num_classes=400,
                           spatial_type='avg',
                           dropout_ratio=0.5),
...
work_dir = './work_dirs/k400/dualformer_tiny_tokenlabel_patch244_window877'

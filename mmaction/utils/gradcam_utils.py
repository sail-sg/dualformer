...
class GradCAM:
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        ...
        weights = weights.reshape(b, tg, c, 1, 1)
        activations = activations.reshape([b, tg, c] +
        ...
        localization_map_min, localization_map_max = (
            torch.min(localization_map.reshape(b, -1), dim=-1, keepdim=True)[0],
            torch.max(localization_map.reshape(b, -1), dim=-1, keepdim=True)[0])
    ...
    def _alpha_blending(self, localization_map, input_imgs, alpha):
        ...
        if self.is_recognizer2d:
            ...
            curr_inp = input_imgs.reshape([-1] + list(input_imgs.size()[2:]))
